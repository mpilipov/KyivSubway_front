...
export default {
  ...
  methods: {
    async searchPosts() {
      this.posts = await axios.get(`http://localhost:8000/api/posts/?search=${this.q}`); // получаем список постов от API
      this.$router.push("/search?q="+this.q); // меняем URL страницы
      this.$emit('searchPosts', this.posts.data); // передаём на страницу search.vue список постов
    },
  }
}
</script>
