      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <form name="contact-form">
            <div class="row">
               <!-- Если ошибки в поле, то применить ко всему блоку класс fld-error -->
              <div class="col-md-6" v-bind:class="{ 'fld-error': $v.form.name.$error }"> 
                ...
                <div class="md-form mb-0">
                  ...
                  @input="$v.form.name.$touch()">
                      <!-- Валидация поля namе будет только после ввода данных туда -->
                </div>
...
<textarea type="text" id="message" rows="2" class="form-control md-textarea" placeholder="Ваше сообщение" v-model="form.message"
@input="$v.form.message.$touch()"></textarea>
          ...
          <div class="text-center text-md-left mt-3">
            ...
            type="submit" @click.prevent="submitForm" :disabled='!isComplete'>Отправить</button>
          </div>
        </div>
      </div>
    </div>
...
import { required, minLength, email } from 'vuelidate/lib/validators'// обязательное поле, мин. длина, валидация email
export default {
  components: {Header},
  layout: "post_detail",
  data() {
  ...
  methods: {
    submitForm() {
      ...
      // собираем все данные в переменную contactFormData
      ...
      this.$router.push("/success"); // редирект на страницу pages/success.vue
  ...
  validations: {
  form: { // перечиалены все 4 поля, которые подлежат проверке
    name: {
      required,
