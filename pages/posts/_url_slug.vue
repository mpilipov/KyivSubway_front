    ...
    <li class="breadcrumb-item active" aria-current="page" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
      ...
      <meta itemprop="position" content="2">
          ...
          <div class="d-flex">
            ...
            <div class="p-2">Опубликовано: {{ post.created }}</div>
          </div>
          <hr />
...
export default {
  components: { Header, Aside, Comments },
  layout: "post_detail", //указываем кастомный layout, не default
  async asyncData({ params }) {
    console.log(params);
    const post = await axios.get(
      `http://127.0.0.1:8000/api/posts/${params.url_slug}`);
    const tags = await axios.get(`http://127.0.0.1:8000/api/tags/`);
    const aside = await axios.get(`http://127.0.0.1:8000/api/aside/`);
    
    const comments = await axios.get(`http://127.0.0.1:8000/api/comments/${params.url_slug}`);
